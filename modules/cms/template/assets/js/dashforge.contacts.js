...
if ($('.contact-sidebar-body').length && contactSidebar == undefined) {
...
// ##################################
appContacts.sidebar = new Vue({
  el: '#contactsListUsers',
  data() {
    return {
      users: appContacts.users,
      groups: []
    };
  },
})

$.get("/api/query/users/").then(response => {
    appContacts.sidebar.users = response
    //appContacts.sidebar.$forceUpdate()
});

/*
$.get("/api/query/users/",function(data){
